...
class ViewFundraisingCampaignModerator extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      campaign: {
  ...
  completeCampaign = async () => {
    const res = await client.postRequestToken(
      "/moderator/completeFundraisingCampaign",
  ...
  approveCampaign = async () => {
    const res = await client.postRequestToken(
      "/moderator/moderationVerifyFundraisingCampaign",
      {
        campaignId: this.state.campaign._id,
        verified: true,
      }
...
function WithNavigate(props) {
  ...
  return <ViewFundraisingCampaignModerator {...props} navigate={navigate} />;
}
