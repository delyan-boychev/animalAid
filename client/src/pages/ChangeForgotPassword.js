...
export default class ChangeForgotPassword extends React.Component
{
    ...
    checkToken = async () =>
    {
        let res = await client.getRequest(`/user/validateForgetPasswordToken/${new URLSearchParams(window.location.search).get("token")}`);
        console.log(res);
    ...
    submitForm = async (event) =>
    {
      ...
      if(this.state.errors.isValid)
      {
          ...
          {
            this.openModal("Не успяхме да променим паролата! Моля опитайте отново! Извиняваме се за неудобството!");
          }
      }
    }
    validate()
    ...
    render()
    {
        return(
            <div>
                {this.state.tokenValid ? (<div><CustomModal show={this.state.modal.show} title={this.state.modal.title} body={this.state.modal.body} closeModal={this.closeModal}></CustomModal>
                ...
                </Card></div>): (<div><h1 className="text-center">Линкът за смяна на паролата е невалиден!</h1><p style={{fontSize: "150px"}} className="text-center text-primary"><FontAwesomeIcon icon={faTimesCircle}></FontAwesomeIcon></p></div>)}
            </div>
        )
